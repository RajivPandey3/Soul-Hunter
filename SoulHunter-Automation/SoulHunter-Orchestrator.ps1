...
if (-not [string]::IsNullOrWhiteSpace($ProviderSnapshotJson)) {
    try {
        $ProviderSnapshot = @($ProviderSnapshotJson | ConvertFrom-Json -ErrorAction Stop)
        if ($ProviderSnapshot.Count -eq 1 -and $ProviderSnapshot[0] -is [System.Array]) {
            $ProviderSnapshot = @($ProviderSnapshot[0])
        }
    }
...
function Test-EvidencePath([string]$Root, [string]$RelativePath) {
    if ([string]::IsNullOrWhiteSpace($RelativePath)) { return $false }
    $p = $RelativePath.Replace("/", "\")
    if (Test-IsExcluded $p) { return $false }
    $full = Join-Path $Root $p
    return (Test-Path -LiteralPath $full -PathType Leaf)
}

function Test-RequiresRuntimeProof([string]$Objective, [string[]]$Checks, [string]$ValidationLevel) {
    $text = ($Objective + " " + ($Checks -join " ")).ToLowerInvariant()
    if ($ValidationLevel -eq "STATIC") {
        foreach ($k in @("playmode", "frame rate", "fps", "runtime feel", "physics feel")) {
...
# Learning Comment: Test-NeedsUnityValidation evaluates whether the exact workspace change delta
# touches Unity assets, scripts, packages, or project settings that mandate Unity CLI compilation
# and import validation before canonical merge.
function Test-NeedsUnityValidation($Changes) {
    if ($null -eq $Changes -or @($Changes).Count -eq 0) { return $false }
    $unityExts = @(".cs", ".unity", ".prefab", ".asset", ".mat", ".controller", ".anim", ".shader", ".hlsl", ".asmdef", ".asmref")
    foreach ($c in @($Changes)) {
        $rel = [string]$c.RelativePath
        $ext = [System.IO.Path]::GetExtension($rel).ToLowerInvariant()
        if ($unityExts -contains $ext) { return $true }
        if ($rel -like "Assets/*" -or $rel -like "Assets\*" -or $rel -like "ProjectSettings/*" -or $rel -like "ProjectSettings\*" -or $rel -like "Packages/*" -or $rel -like "Packages\*") {
            return $true
        }
    }
    return $false
...
# Learning Comment: Quote-CliArg ensures command-line arguments are properly escaped and quoted
# for Windows process execution, guarding against spaces and path separators.
function Quote-CliArg([string]$Value) {
    return ConvertTo-WindowsProcessArgument $Value
}
...
function Get-EligibleSemanticBackends {
    ...
    if (@($Providers).Count -eq 1 -and $Providers[0] -is [System.Array]) {
    ...
    foreach ($p in $rows) {
        ...
        if ([string]$p.name -like "AGY*") {
            if ($AgyExe -and $p.model_id) { $result += [pscustomobject]@{Name=$safeName;Exe=$AgyExe;ModelId=[string]$p.model_id;Provider=$p} }
...
function Get-ObjectiveGoalFingerprint {
    param(
        [Parameter(Mandatory=$true)]
        [string]$RootGoal
    )

    if ([string]::IsNullOrWhiteSpace($RootGoal)) {
        throw "Objective ledger requires a non-empty root goal."
    }

    $normalized = $RootGoal.Trim()
...
function Get-ObjectiveLedgerPath {
    $ledgerDir = Join-Path $AutomationRoot "state"

    if (-not (Test-Path $ledgerDir)) {
        New-Item -ItemType Directory -Path $ledgerDir -Force | Out-Null
    }

    return (Join-Path $ledgerDir "objective-ledger.jsonl")
}

function Read-ObjectiveLedger {
    param(
        [Parameter(Mandatory=$true)]
        [string]$RootGoal
    )

    $path = Get-ObjectiveLedgerPath

    if (-not (Test-Path $path)) {
        return @()
    }

    $goalFingerprint = Get-ObjectiveGoalFingerprint $RootGoal
    $result = @()
    $lineNumber = 0
    ...
    foreach ($line in (Get-Content -LiteralPath $path -ErrorAction Stop)) {
        ...
        try {
            ...
            throw "Objective ledger is malformed at line $lineNumber. Refusing to continue."
        }

        if ([string]::IsNullOrWhiteSpace([string]$entry.objective_id) -or
            [string]::IsNullOrWhiteSpace([string]$entry.status) -or
            [string]::IsNullOrWhiteSpace([string]$entry.objective_text) -or
            [string]::IsNullOrWhiteSpace([string]$entry.goal_fingerprint)) {
            throw "Objective ledger entry at line $lineNumber is missing required fields. Refusing to continue."
        }

        if ([string]$entry.goal_fingerprint -eq $goalFingerprint) {
            $result += $entry
        }
    }
...
function Add-ObjectiveLedgerEntry {
    param(
        [Parameter(Mandatory=$true)][string]$RootGoal,
        [Parameter(Mandatory=$true)][string]$ObjectiveId,
        [Parameter(Mandatory=$true)][string]$ObjectiveText,
    ...
    $normalizedId = $ObjectiveId.Trim().ToUpperInvariant()
    $normalizedStatus = $Status.Trim().ToUpperInvariant()

    if ([string]::IsNullOrWhiteSpace($normalizedId)) {
        throw "Objective ledger rejected an empty objective ID."
    }

    if ([string]::IsNullOrWhiteSpace($ObjectiveText)) {
        throw "Objective ledger rejected empty objective text."
    }

    if ($normalizedStatus -notin $allowedStatuses) {
        throw "Objective ledger rejected unknown status '$normalizedStatus'."
    }
    ...
    $entry = [ordered]@{
        ...
        goal_fingerprint = Get-ObjectiveGoalFingerprint $RootGoal
        objective_id     = $normalizedId
...
function Get-TaskLedgerPath {
    $ledgerDir = Join-Path $AutomationRoot "state"
    if (-not (Test-Path $ledgerDir)) {
        New-Item -ItemType Directory -Path $ledgerDir -Force | Out-Null
    }
    return (Join-Path $ledgerDir "task-ledger.jsonl")
}

function Read-TaskLedger {
    $path = Get-TaskLedgerPath
...
function ConvertFrom-AgentJson([string]$Text) {
    # Extract and parse JSON payload from agent output, handling markdown code fences or raw JSON
    if ([string]::IsNullOrWhiteSpace($Text)) { return $null }
    $clean = $Text.Trim()
    if ($clean -match '(?s)```(?:json)?\s*(?<json>\[.*?\]|\{.*?\})\s*```') {
        $clean = $Matches.json.Trim()
    } elseif ($clean -match '(?s)(?<json>\[\s*\{.*\}\s*\]|\{\s*".*"\s*:.*\})') {
        $clean = $Matches.json.Trim()
    }
    try {
...
function Invoke-ScannerJsonRecovery(
    [string]$AgyExe,
    [string]$CodexExe,
    [string]$Workspace,
    [string]$EvidenceDir,
    ...
    # If the LLM returned text containing a partial or slightly malformed JSON,
    # invoke an available semantic model in recovery mode to emit strictly valid JSON.
    $prompt = @"
You are a deterministic JSON recovery assistant.
Extract and output ONLY the valid JSON data structure from the following text according to schema '$SchemaKind'.
Do NOT add explanations, markdown formatting, or comments.
TEXT:
$RawText
"@
    $log = Join-Path $EvidenceDir ("recovery-" + $SchemaKind.ToLowerInvariant() + ".log")
    $r = Invoke-Astra $AgyExe $CodexExe $prompt $Workspace $log
    if ($r.ExitCode -eq 0) {
        return (Get-FinalAgentText $r.Output)
    }
    return ""
}
...
function Get-NextObjectiveFromProjectScan(
    ...
    $ledger = @(Read-ObjectiveLedger -RootGoal $RootGoal)
...
function Invoke-Autopilot([string]$RootGoal) {
    ...
    while ($true) {
        ...
        if ($null -eq $currentObjective) {
            ...
            Add-ObjectiveLedgerEntry $RootGoal $currentObjectiveId $currentObjective "ACTIVE" "Discovered by project scan."
        ...
        if ($taskId) {
            if ($seen.ContainsKey($taskId)) {
                ...
                Add-ObjectiveLedgerEntry $RootGoal $currentObjectiveId $currentObjective "EXHAUSTED" "Repeated task loop detected."
                $currentObjective = $null
                $currentObjectiveId = $null
                continue
            }
        ...
        if ($status -eq "NO_SAFE_TASK") {
            ...
            Add-ObjectiveLedgerEntry $RootGoal $currentObjectiveId $currentObjective "EXHAUSTED" "No safe tasks remain."
            $currentObjective = $null
            $currentObjectiveId = $null
            continue
        }

        if ($status -eq "OWNER_BLOCKED") {
            Write-Host "Objective level OWNER_BLOCKED -> stopping for owner input."
            Add-ObjectiveLedgerEntry $RootGoal $currentObjectiveId $currentObjective "OWNER_BLOCKED" "Task executor requires owner input."
            $stopReason = "OWNER_INPUT_REQUIRED"
            break
        }

        if ($status -in @("APPLIED","NO_CHANGE","CANDIDATE_REJECTED","TASK_BLOCKED")) {
            Write-Host "Next action : FRESH PROJECT OBJECTIVE SCAN"
            $currentObjective = $null
            $currentObjectiveId = $null
            continue
        }
