...
namespace SoulHunter.Gameplay.UI
{
    ...
    public class ChestUI : MonoBehaviour
    {
        ...
        public void OpenChest()
        {
            ...
            // Reward random upgrade
            if (_weaponManager != null && _availableUpgrades != null && _availableUpgrades.Count > 0)
            {
                int rand = Random.Range(0, _availableUpgrades.Count);
                UpgradeData randomUpgrade = _availableUpgrades[rand];
                
                _weaponManager.ApplyUpgrade(randomUpgrade);
                
                if (_rewardText != null)
                {
                    _rewardText.text = "You Found a Chest!\n\n<color=yellow>Upgraded: " + randomUpgrade.UpgradeName + "</color>";
                }
            }
            else
            {
                if (_rewardText != null) _rewardText.text = "You Found a Chest!\n\n<color=yellow>Gold: +100</color>";
            }
        }
